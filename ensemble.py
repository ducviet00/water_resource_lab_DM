...
class Ensemble:
    ...
    def build_model_inner(self):
        ...
        model = model_builder(self.input_dim,self.output_dim,self.target_timestep)
        model.save_weights(self.log_dir + 'ModelPool/init_model.hdf5')
        return model

    def train_model_inner(self):
        # x_train_out = pd.DataFrame(self.data['x_test_in'].reshape(-3,self.window_size * self.input_dim))
        # x_test_out = pd.DataFrame(self.data['x_test_out'].reshape(-3,self.window_size * self.input_dim))
    ...
    def build_model_outer(self):
        ...
        rnn_2 = LSTM(units=256,return_sequences=False)
        rnn_2_out = rnn_2(input_submodel,initial_state=[state_h,state_c])
    ...
    def train_model_outer(self):
        if (self.mode == 'train' or self.mode == 'train-outer'):
            ...
            history = self.outer_model.fit(x=[self.data['x_train_out'][:,0,:,:],self.data['x_test_in']],
                                     ...
                                     batch_size=self.batch_size,epochs=self.epochs_out,
                                     callbacks=callbacks,
...
if __name__ == '__main__':
    ...
    if args.mode == 'train' or args.mode == 'train-inner' or args.mode == 'train-outer':
        model = Ensemble(args.mode,**config)
