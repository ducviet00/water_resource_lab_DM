from tensorflow.keras.layers import Dense,LSTM,Bidirectional,Input,Concatenate,Conv1D, TimeDistributed
from attention_implement import AttentionLayer
...
class encodeDecodeAttention:
    ...
    def build_model(self):
        encoder_inputs = Input(batch_shape=(self.batch_size, self.window_size, self.input_dim))

        encoder = Bidirectional(LSTM(256,return_sequences=True, return_state=True, dropout=self.dropout))
        ...
        att_out, att_states = attention_layer([encoder_outputs,decoder_out])
...
if __name__ == '__main__':
    ...
    with open('./Config/EncoderDecoder/Attention/config.yaml','r') as f:
        config = yaml.load(f,Loader=yaml.FullLoader)
    if args.mode == 'train':
        att_model = encodeDecodeAttention(args.mode,**config)
        att_model.train_model()
        att_model.evaluate_model()
